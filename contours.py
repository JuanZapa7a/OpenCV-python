...
for i in range(0, len(hierarchy[0])):
    ...
    cv2.drawContours(
        ...
        cv2.FILLED, 1, hierarchy
    )

cv2.imshow('image',im)

# cv2.imshow() to display an image in a window. The window automatically fits
# to the image size.
...
# cv2.destroyAllWindows() simply destroys all the windows we created. If you
# want to destroy any specific window, use the function cv2.destroyWindow()
